...
public class BudgetControllerTest {
    ...
    public void setup() {
        ...
        budgetDto = BudgetDto.builder().build();
    ...
    public void getAllBudgetsTest() {
        // given
        when(budgetService.getAll()).thenReturn(budgets);
    ...
    public void createBudgetTest() {
        ...
        when(budgetService.create(user, budgetDto)).thenReturn(budget);

        // when
        ResponseEntity<Budget> response = budgetController.createBudget(budgetDto);
    ...
    public void updateBudgetTest() {
        // given
        when(budgetService.update(1L, budgetDto)).thenReturn(budget);

        // when
        ResponseEntity<Budget> response = budgetController.updateBudget(1L, budgetDto);
    ...
    public void filterBudgetsTest() {
        // when
        when(userService.findByUsername(budgetDto.getUsername())).thenReturn(user);
        when(budgetService.filterBudgets(user, filterDto)).thenReturn(budgets);

        // when
        ResponseEntity<List<Budget>> response = budgetController.filterBudgets(filterDto);
    ...
    public void filterByUserTest() {
        ...
        //when(budgetService.findByUser(user)).thenReturn(budgets);

        // when
        //ResponseEntity<List<Budget>> response = budgetController.filterByUser(filterDto);
